...
void main() {
  ...
  pw.Widget _buildSummaryAveragesTable(Map<dynamic, dynamic> nets, Map<dynamic, dynamic> counts) {
    ...
    List<pw.TableRow> rows = [
      pw.TableRow(children: ['', ...subjects].map((s) => _cell(s, bold: true)).toList()),
      pw.TableRow(children: [
        _cell('ORTALAMA SORU SAYISI', bold: true),
        _cell(totalQ.toString()),
        ...subjects.skip(1).map((s) {
           final key = counts.keys.firstWhere((k) => k.toString().toUpperCase().contains(s), orElse: () => null);
           return _cell(counts[key]?.toString() ?? '-');
        })
      ]),
      pw.TableRow(children: [
        _cell('ORTALAMA NET', bold: true),
        _cell(totalNet.toStringAsFixed(2)),
        ...subjects.skip(1).map((s) {
           final key = nets.keys.firstWhere((k) => k.toString().toUpperCase().contains(s), orElse: () => null);
           return _cell((nets[key] as double? ?? 0.0).toStringAsFixed(2));
        })
      ]),
    ];
        ...
        pw.Table(
          ...
          children: [
            pw.TableRow(
              ...
              children: [
                pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text('ANALİZ TÜRÜ', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9, color: PdfColors.purple900))),
                pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text('DEĞER', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9, color: PdfColors.purple900))),
              ]
            ),
            pw.TableRow(children: [
              pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text('GENEL PUAN ORTALAMASI', style: const pw.TextStyle(fontSize: 10))),
              pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text((summary['avgPoint'] as double? ?? 0.0).toStringAsFixed(2), style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 12))),
            ]),
            pw.TableRow(children: [
              pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text('GENEL NET ORTALAMASI', style: const pw.TextStyle(fontSize: 10))),
              pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text((summary['avgNet'] as double? ?? 0.0).toStringAsFixed(2), style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 12))),
            ]),
          ]
        ),
  ...
  pw.Widget _buildTrialExamsSection(Map<String, dynamic> data, bool newPage, pw.Font fontBold) {
    ...
    List<pw.TableRow> examRows = [
      pw.TableRow(
        ...
        children: ['Sınav Adı', 'SS', ...subjects].map((s) => pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(s, style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 8)))).toList()
      )
    ];

    for (int i = 0; i < trialExams.length; i++) {
        final exam = trialExams[i];
        final subNets = exam['subjects'] as Map? ?? {};
        examRows.add(pw.TableRow(
            children: [
                pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('\${i+1}. \${exam['examName']}', style: const pw.TextStyle(fontSize: 8))),
                pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(exam['totalQuestions']?.toString() ?? '-', style: const pw.TextStyle(fontSize: 8))),
                pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(exam['net']?.toStringAsFixed(1) ?? '-', style: const pw.TextStyle(fontSize: 8))),
                ...subjects.skip(1).map((s) {
                    final key = subNets.keys.firstWhere((k) => k.toString().toUpperCase().contains(s), orElse: () => null);
                    return pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(key != null ? (subNets[key]['net'] ?? '-').toString() : '-', style: const pw.TextStyle(fontSize: 8)));
                })
            ]
        ));
    }
    ...
    examRows.add(pw.TableRow(
      decoration: const pw.BoxDecoration(color: PdfColors.grey100),
      children: [
        pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text('ORTALAMA', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 8))),
        pw.Text(''),
        pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(summary['avgNet']?.toStringAsFixed(2) ?? '-', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 8))),
        ...subjects.skip(1).map((s) {
           final key = (summary['subjectAvgNets'] as Map?)?.keys.firstWhere((k) => k.toString().toUpperCase().contains(s), orElse: () => null);
           return pw.Padding(padding: const pw.EdgeInsets.all(4), child: pw.Text(key != null ? (summary['subjectAvgNets'][key] as double).toStringAsFixed(2) : '-', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 8)));
        })
      ]
    ));
...
""";
  ...
  if (startIdx != -1 && endIdx != -1) {
    String finalContent = content.substring(0, startIdx) + finalMethods + "\n\n  " + content.substring(endIdx);
