...
class DepressiveTendencyReport extends StatefulWidget {
  final Survey survey;
  final List<Map<String, dynamic>> responses;
  final Map<String, String> userNames;

  const DepressiveTendencyReport({
    Key? key,
  ...
  @override
  State<DepressiveTendencyReport> createState() =>
      _DepressiveTendencyReportState();
}

class _DepressiveTendencyReportState extends State<DepressiveTendencyReport>
  ...
  final Map<String, List<int>> _categories = {
    'emotional': List.generate(6, (i) => i + 1), // q1-q6
    'interest': List.generate(6, (i) => i + 7), // q7-q12
    'energy': List.generate(6, (i) => i + 13), // q13-q18
    'cognitive': List.generate(6, (i) => i + 19), // q19-q24
    'self': List.generate(6, (i) => i + 25), // q25-q30
    'social': List.generate(6, (i) => i + 31), // q31-q36
    'numbness': List.generate(9, (i) => i + 37), // q37-q45
  };

  final Map<String, String> _categoryNames = {
    'emotional': 'Duygusal Çöküntü',
    'interest': 'İlgi ve Zevk Kaybı',
    'energy': 'Enerji ve Motivasyon',
    'cognitive': 'Bilişsel Yavaşlama',
    'self': 'Kendilik Algısı',
    'social': 'Sosyal Geri Çekilme',
    'numbness': 'Duygusal Donukluk',
  };

  final List<int> _reverseItems = [4, 9, 16, 22, 26, 29, 34];
  ...
  Widget _buildScoreOverview(double score, int count, double indecisiveRatio) {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            const Text('Genel Risk Düzeyi', style: TextStyle(fontSize: 16)),
              ...
              Container(
                ...
                child: const Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Yüksek Kararsızlık Oranı: Bireyde duygusal farkındalık eksikliği veya bastırma eğilimi söz konusu olabilir.',
                        style: TextStyle(
                          fontSize: 12,
                          fontWeight: FontWeight.bold,
                        ),
  ...
  Widget _buildRadarChart(Map<String, double> averages) {
    ...
    final data = cats.map((cat) {
      ...
      final max = cat == 'numbness' ? 36.0 : 24.0;
  ...
  String _generateAdviceString(Map<String, double> averages) {
    ...
    final interest = averages['interest'] ?? 0;
    final energy = averages['energy'] ?? 0;
    final cognitive = averages['cognitive'] ?? 0;
    final self = averages['self'] ?? 0;
    final social = averages['social'] ?? 0;
    final numbness = averages['numbness'] ?? 0;
    final indecisiveRatio = averages['indecisiveRatio'] ?? 0;

    String advice = "DEPRESİF EĞİLİM UZMAN ANALİZ RAPORU\n\n";

    advice += "A. Genel Profil Değerlendirmesi\n";
    if (totalScore <= 45) {
      advice +=
          "Bireyin mevcut psikolojik tablosu, sağlıklı duygu durum dengesini koruyabildiğini ve yaşamsal motivasyonunun yüksek olduğunu göstermektedir.\n\n";
    } else if (totalScore <= 90) {
      advice +=
          "Analiz sonuçları, hafif düzeyde bir ruhsal yorgunluğa ve geçici duygu durum dalgalanmalarına işaret etmektedir.\n\n";
    } else if (totalScore <= 135) {
      advice +=
          "Bireyin orta düzeyde bir depresif eğilim örüntüsü içinde olduğu gözlemlenmektedir. Bireyin günlük işlevselliğini zorlamaya başlayan bir isteksizlik hali söz konusudur.\n\n";
    } else {
      advice +=
          "Birey, çok yüksek seviyede depresif risk tablosu sergilemektedir. Acilen profesyonel bir değerlendirme planlanmalıdır.\n\n";
    }

    advice += "B. Alt Boyutlara Dayalı Detaylı Analiz\n";
    if (numbness > 18 || emotional > 15 || interest > 15) {
      advice += "Profilde özellikle ";
      if (numbness > 18) advice += "'duygusal donukluk' baskındır. ";
      if (emotional > 15) advice += "Duygusal çöküntü belirgindir. ";
      if (interest > 15) advice += "İlgi ve zevk kaybı ön plandadır. ";
      if (indecisiveRatio > 30)
        advice +=
            "Yüksek kararsızlık, duygularıyla yüzleşmekten çekindiğini teyit etmektedir.\n\n";
      else
        advice += "\n\n";
    } else {
      advice +=
          "Alt boyutlar arasındaki dağılım, bireyin hala belirli alanlarda kapasitesini koruduğunu göstermektedir.\n\n";
    }

    advice += "C. Günlük Yaşam ve Akademik/Sosyal Etkilere Yansıma\n";
    if (energy > 15 || cognitive > 15 || social > 15) {
      advice += "Mevcut profil, bireyin ";
      if (energy > 15)
        advice += "sabahları güne başlama direncini artırmaktadır. ";
      if (cognitive > 15)
        advice +=
            "odaklanma güçlüğü nedeniyle akademik performansını etkilemektedir. ";
      if (social > 15)
        advice += "sosyal izolasyon eğilimini tetiklemektedir.\n\n";
    } else {
      advice +=
          "Günlük işlevsellik üzerinde henüz yıkıcı bir etki gözlemlenmemiştir.\n\n";
    }

    advice += "D. Öneriler\n";
    advice +=
        "Bireyin 'davranışsal aktivasyon' ilkesine uygun olarak, küçük ama istikrarlı adımlarla günlük rutinler oluşturması önerilir. ";
    if (self > 15)
      advice +=
          "Öz-şefkat çalışmaları ve bilişsel yeniden yapılandırma faydalı olacaktır.\n\n";
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          child: ListTile(
            ...
            subtitle: Text('Puan: ${stats['total']?.toInt()} / 180'),
  ...
  Future<void> _exportSummaryToPdf(
    ...
    try {
      ...
      final pdfBytes = await pdfService.generateSurveyReportPdf(
        title: 'Depresif Eğilim Ölçeği (DEÖ)',
        subTitle: subTitle,
  ...
  Future<void> _exportTableToExcel(List<Map<String, dynamic>> filtered) async {
    try {
      ...
      var sheet = excel['DEO Sonuçları'];
      sheet.appendRow([
        TextCellValue('Öğrenci Adı'),
        TextCellValue('Toplam Puan'),
        TextCellValue('Duygusal'),
        TextCellValue('İlgi Kaybı'),
        TextCellValue('Enerji'),
        TextCellValue('Bilişsel'),
        TextCellValue('Kendilik Algısı'),
        TextCellValue('Sosyal Çekilme'),
        TextCellValue('Donukluk'),
      ]);
