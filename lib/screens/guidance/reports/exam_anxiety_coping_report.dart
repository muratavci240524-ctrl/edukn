...
class ExamAnxietyCopingReport extends StatefulWidget {
  final Survey survey;
  final List<Map<String, dynamic>> responses;
  final Map<String, String> userNames;

  const ExamAnxietyCopingReport({
    Key? key,
  ...
  @override
  State<ExamAnxietyCopingReport> createState() =>
      _ExamAnxietyCopingReportState();
}

class _ExamAnxietyCopingReportState extends State<ExamAnxietyCopingReport>
  ...
  final Map<String, List<int>> _categories = {
    'recon': List.generate(6, (i) => i + 1), // q1-q6
    'cog': List.generate(6, (i) => i + 7), // q7-q12
    'phys': List.generate(6, (i) => i + 13), // q13-q18
    'study': List.generate(6, (i) => i + 19), // q19-q24
    'avoid': List.generate(6, (i) => i + 25), // q25-q30
    'emotional': List.generate(6, (i) => i + 31), // q31-q36
    'sabotage': List.generate(6, (i) => i + 37), // q37-q42
    'distractor': List.generate(10, (i) => i + 43), // q43-q52
  };

  final Map<String, String> _categoryNames = {
    'recon': 'Kaygıyı Tanıma ve Kabul',
    'cog': 'Bilişsel Baş Etme',
    'phys': 'Bedensel Baş Etme',
    'study': 'Çalışma ve Planlama',
    'avoid': 'Kaçınma ve Erteleme',
  ...
  Map<String, double> _calculateStudentStats(Map<String, dynamic> response) {
    ...
    stats['indecisiveRatio'] = (indecisiveCount / 52.0) * 100;
  ...
  Widget _buildScoreOverview(double score, int count, double indecisiveRatio) {
    ...
    Color color;
    if (score <= 65) {
      level = 'Etkili Baş Etme';
      color = Colors.green;
    } else if (score <= 120) {
      level = 'Geliştirilebilir Baş Etme';
      color = Colors.orange;
    } else if (score <= 170) {
      level = 'Zayıf Baş Etme';
      color = Colors.deepOrange;
    } else {
      level = 'Yüksek Risk / Destek İhtiyacı';
      color = Colors.red;
    }
              ...
              Container(
                ...
                child: const Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Yüksek Kararsızlık Oranı: Baş etme stratejileri henüz oturmamış veya dalgalı bir profil sergilenmektedir.',
                        style: TextStyle(
                          fontSize: 12,
                          fontWeight: FontWeight.bold,
                        ),
  ...
  String _generateAdviceString(Map<String, double> averages) {
    final totalScore = averages['total'] ?? 0;
    final cog = averages['cog'] ?? 0;
    final phys = averages['phys'] ?? 0;
    final avoid = averages['avoid'] ?? 0;
    final sabotage = averages['sabotage'] ?? 0;
    final distractor = averages['distractor'] ?? 0;

    String advice = "SINAV KAYGISI BAŞ ETME ANALİZİ\n\n";

    if (distractor > 25 && totalScore < 100) {
      advice +=
          "ÖNEMLİ: Çeldirici maddelerdeki yüksek puanlar, kaygıyı küçümseme veya inkâr eğilimine işaret ediyor olabilir. Birey gerçek duygularını bastırıyor olabilir.\n\n";
    }

    advice += "A. Profil Değerlendirmesi\n";
    if (totalScore <= 65) {
      advice +=
          "Birey, sınav kaygısını yönetme konusunda etkili stratejilere sahiptir. Kaygıyı bir motivasyon aracı olarak kullanabilmektedir.\n\n";
    } else if (totalScore <= 120) {
      advice +=
          "Bireyin baş etme becerileri gelişim aşamasındadır. Belirli alanlarda (özellikle zaman baskısı altında) zorlanmalar yaşamaktadır.\n\n";
    } else {
      advice +=
          "Birey yüksek düzeyde sınav kaygısı yaşamaktadır ve mevcut baş etme yöntemleri yetersiz kalmaktadır.\n\n";
    }

    advice += "B. Stratejik Analiz\n";
    if (cog > 15)
      advice +=
          "- Bilişsel alanda olumsuz iç konuşmalar ve felaketleştirme senaryoları baskındır.\n";
    if (phys > 15)
      advice +=
          "- Kaygı anında bedensel tepkileri (çarpıntı, terleme) yönetmekte zorlanmaktadır.\n";
    if (avoid > 15)
      advice +=
          "- Kaçınma ve erteleme davranışları akademik hazırlık sürecini sabote etmektedir.\n";
    if (sabotage > 15)
      advice += "- Mükemmeliyetçilik ve kendini eleştirme düzeyi yüksektir.\n";

    advice += "\nC. Öneriler\n";
    advice +=
        "1. Kaygıyı 'düşman' değil, bir 'sinyal' olarak kabul etme çalışmaları yapılmalıdır.\n";
    if (phys > 15)
      advice +=
          "2. Diyafram nefesi ve gevşeme egzersizleri günlük rutine eklenmelidir.\n";
    if (cog > 15)
      advice +=
          "3. Bilişsel yeniden yapılandırma (olumsuz düşünceleri sorgulama) eğitimi verilmelidir.\n";
    if (avoid > 15)
      advice +=
          "4. Zaman yönetimi ve 'küçük adımlar' kuralı ile çalışma düzeni desteklenmelidir.\n";
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          child: ListTile(
            ...
            subtitle: Text('Puan: ${stats['total']?.toInt()} / 208'),
  ...
  Future<void> _exportSummaryToPdf(
    ...
    try {
      ...
      final pdfBytes = await pdfService.generateSurveyReportPdf(
        title: 'Sınav Kaygısı Baş Etme Ölçeği (SKBÖ)',
        subTitle: subTitle,
  ...
  Future<void> _exportTableToExcel(List<Map<String, dynamic>> filtered) async {
    try {
      ...
      var sheet = excel['SKBO Sonuçları'];
      sheet.appendRow([
        TextCellValue('Öğrenci Adı'),
        TextCellValue('Toplam Puan'),
        TextCellValue('Tanıma/Kabul'),
        TextCellValue('Bilişsel'),
        TextCellValue('Bedensel'),
      ...
      for (var r in filtered) {
        ...
        sheet.appendRow([
          ...
          DoubleCellValue(stats['recon'] ?? 0),
          DoubleCellValue(stats['cog'] ?? 0),
          DoubleCellValue(stats['phys'] ?? 0),
          DoubleCellValue(stats['study'] ?? 0),
          DoubleCellValue(stats['avoid'] ?? 0),
