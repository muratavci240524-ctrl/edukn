...
class _AgmCycleSetupScreenState extends State<AgmCycleSetupScreen>
  ...
  int? _maxSaat;
  int? _minDers;
  bool _isSpecialClassEnabled = false;
  ...
  void initState() {
    ...
    if (widget.initialCycle != null) {
      ...
      _isSpecialClassEnabled = c.isSpecialClassEnabled;
      ...
      child: Column(
        ...
        children: [
          ...
          _sectionHeader(
            'Özel Sınıf Ayarları',
            Icons.star_outline,
            subtitle: 'En başarılı öğrencileri tek bir derslikte topla',
          ),
          const SizedBox(height: 12),
          Container(
            ...
            child: Column(
              children: [
                SwitchListTile(
                  ...
                  subtitle: const Text(
                    'Top puanlı öğrenciler belirlenen kapasiteye göre ayrılır.',
                    style: TextStyle(fontSize: 12),
                  ),
                  value: _isSpecialClassEnabled,
                  activeColor: Colors.deepOrange,
                  onChanged: (v) => setState(() => _isSpecialClassEnabled = v),
                  contentPadding: EdgeInsets.zero,
                ),
                if (_isSpecialClassEnabled) ...[
                  ...
                  Row(
                    children: [
                      Expanded(
                        flex: 2,
                        child: TextFormField(
                          initialValue: _specialClassCapacity?.toString(),
                          keyboardType: TextInputType.number,
                          decoration: _inputDecoration('Kontenjan'),
                          onChanged: (v) => _specialClassCapacity = int.tryParse(v),
                        ),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        ...
                        child: DropdownButtonFormField<String>(
                          value: _specialClassRoomId,
                          decoration: _inputDecoration('Derslik'),
                          hint: const Text('Derslik Seçin', style: TextStyle(fontSize: 13)),
                          ...
                          onChanged: (v) {
                            setState(() {
                              _specialClassRoomId = v;
                              if (v != null) {
                                _specialClassRoomName = _classrooms.firstWhere((c) => c['id'] == v)['name'];
                              } else {
                                _specialClassRoomName = null;
                              }
                            });
                          },
                        ),
        ...
        children: [
          ...
          if (_isSpecialClassEnabled)
            _buildSummaryItem(
              icon: Icons.star,
              label: 'Özel Sınıf',
              value: '${_specialClassCapacity ?? '—'} Kişi • ${_specialClassRoomName ?? 'Derslik Seçilmedi'}',
              ok: _specialClassCapacity != null && _specialClassRoomId != null,
            ),
          const SizedBox(height: 24),
  ...
  Future<void> _performSave(Stopwatch sw) async {
    ...
    // 1.1) Özel Sınıf gruplarını ekle (Eğer aktifse her slot için 1 tane)
    if (_isSpecialClassEnabled && _specialClassRoomId != null) {
      for (final slot in _existingSlots) {
        proposedGroups.add(
          AgmGroup(
            id: '',
            ...
            bitisSaat: slot.bitisSaat,
            ogretmenId: '',
            ogretmenAdi: 'Öğretmen Atanmadı',
            derslikId: _specialClassRoomId,
            derslikAdi: _specialClassRoomName,
            kapasite: _specialClassCapacity ?? 24,
            mevcutOgrenciSayisi: 0,
            isSpecial: true,
          ),
        );
    ...
    final cycle = AgmCycle(
      ...
      isSpecialClassEnabled: _isSpecialClassEnabled,
