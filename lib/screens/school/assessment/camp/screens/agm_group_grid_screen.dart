...
class _AgmGroupGridScreenState extends State<AgmGroupGridScreen>
  ...
  Widget _buildGroupsTab() {
    ...
    final Set<String> allGroupBranches = _groups.where((g) => !g.isSpecial).map((g) => g.dersAdi).toSet();
    final bool hasSpecialGroups = _groups.any((g) => g.isSpecial);
    ...
    for (final g in _groups) {
      if (_groupFilterBranch != null) {
        if (_groupFilterBranch == 'Özel Sınıf') {
          if (!g.isSpecial) continue;
        } else {
          if (g.dersAdi != _groupFilterBranch || g.isSpecial) continue;
        }
      }
    ...
    // Branşları alfabetik sırala (Özel Sınıf en üstte olsun)
        ...
        Container(
          ...
          child: DropdownButton<String>(
            ...
            items: [
              ...
              if (_groups.any((g) => g.isSpecial))
                const DropdownMenuItem<String>(
                  value: 'Özel Sınıf',
                  child: Text('Özel Sınıf ⭐', style: TextStyle(fontWeight: FontWeight.bold)),
                ),
              ...branches.map(
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        leading: Container(
          ...
          decoration: BoxDecoration(
            color: group.isSpecial ? Colors.amber.shade50 : Colors.deepOrange.shade50,
            borderRadius: BorderRadius.circular(8),
          ),
          child: Icon(
            group.isSpecial ? Icons.stars : Icons.book_outlined,
            color: group.isSpecial ? Colors.amber.shade700 : Colors.deepOrange,
            size: 22,
          ),
        ),
        title: Text(
