...
class AgmAssignmentEngine {
    ...
    for (final profil in siralanmisProfillar) {
      ...
      for (final dersEntry in siraliDersler) {
        ...
        final findResult = _findBestEfficientGroupWithReason(
          ...
          isSpecialStudent: specialStudentIds.contains(ogrenciId),
  ...
  _FindGroupResult _findBestEfficientGroupWithReason({
    ...
    bool isSpecialStudent = false,
    String? specialClassRoomId,
  }) {
    final dersGruplari = gruplar.where((g) {
      if (isSpecialStudent) {
        // Özel öğrenci: Kendi grubunu veya dersin grubunu görebilir (Özel grup öncelikli olacak)
        return g.isSpecial || g.dersId == dersId || g.dersAdi == dersId;
      }
      // Normal öğrenci: Sadece branş gruplarını görebilir, özel grupları göremez
      return !g.isSpecial && (g.dersId == dersId || g.dersAdi == dersId);
    }).toList();
    ...
    for (final g in dersGruplari) {
      ...
      // Özel Sınıf Kuralı:
      // Eğer öğrenci özel sınıftaysa ve derslik belirlenmişse, o derslikteki gruba gitmeli.
      if (isSpecialStudent && specialClassRoomId != null) {
        if (g.derslikId != specialClassRoomId) {
          // Bu grup özel derslikte değil, atla (eğer o derslikte başka grup varsa)
          if (dersGruplari.any((dg) => dg.derslikId == specialClassRoomId && !ogrenciDoluSlot.contains(dg.saatDilimiId) && (grupMevcut[dg.id] ?? 0) < dg.kapasite)) {
            continue;
          }
        }
      } else if (!isSpecialStudent && specialClassRoomId != null) {
        // Eğer öğrenci özel sınıfta DEĞİLSE, özel derslikteki gruplara girmemeli (eğer başka seçenek varsa)
        if (g.derslikId == specialClassRoomId) {
          if (dersGruplari.any((dg) => dg.derslikId != specialClassRoomId && !ogrenciDoluSlot.contains(dg.saatDilimiId) && (grupMevcut[dg.id] ?? 0) < dg.kapasite)) {
            continue;
          }
