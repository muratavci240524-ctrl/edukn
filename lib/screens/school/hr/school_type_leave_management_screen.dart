...
class SchoolTypeLeaveManagementScreen extends StatefulWidget {
  ...
  const SchoolTypeLeaveManagementScreen({
    super.key,
    this.institutionId,
    this.schoolTypeId,
  });
  ...
  @override
  State<SchoolTypeLeaveManagementScreen> createState() => _SchoolTypeLeaveManagementScreenState();
}

class _SchoolTypeLeaveManagementScreenState extends State<SchoolTypeLeaveManagementScreen>
    ...
    try {
      ...
      if (mounted) {
        ...
        allRequests.sort((a, b) {
          final dateA = _toDate(a['startDate'] ?? a['createdAt']);
          final dateB = _toDate(b['startDate'] ?? b['createdAt']);
  ...
  Widget _buildRequestList(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final start = _toDate(req['startDate']);
        final end = _toDate(req['endDate']);
            ...
            onTap: isManager && isPending
                ? () async {
                    final result = await Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => SchoolTypeLeaveApprovalScreen(
                          request: {
                            ...req,
                            'id': req['id'],
                            'staffName': _getStaffName(req['staffId'] ?? req['userId']),
                          },
                          institutionId: instId ?? '',
                        ),
                      ),
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              _getStaffName(req['staffId'] ?? req['userId']),
                              style: GoogleFonts.poppins(
                                fontWeight: FontWeight.bold,
                                fontSize: 16,
                                color: const Color(0xFF0F172A),
  ...
  Future<void> _exportToExcel() async {
    try {
      ...
      for (var r in [..._pendingRequests, ..._historyRequests]) {
        sheet.appendRow([
          ex.TextCellValue(_getStaffName(r['staffId'] ?? r['userId'])),
          ex.TextCellValue(r['type'] ?? ''),
          ex.TextCellValue(
            DateFormat(
              'dd.MM.yyyy',
              'tr_TR',
            ).format(_toDate(r['startDate'])),
          ),
          ex.TextCellValue(
            DateFormat(
              'dd.MM.yyyy',
              'tr_TR',
            ).format(_toDate(r['endDate'])),
          ),
  ...
  /// Converts a Firestore date field (Timestamp or String 'yyyy-MM-dd') to DateTime
  ...
  String _getStaffName(String? id) {
    if (id == null || id.isEmpty) return 'Bilinmeyen';
