...
class _ToDoListScreenState extends State<ToDoListScreen>
  ...
  // TabController nullable — data yüklendikten sonra oluşturulur, bir kez
  ...
  Future<void> _loadData() async {
    try {
      // 1. Kullanıcı verisi — cache varsa anında, yoksa Firestore'dan
      final cached = UserPermissionService.getCachedUserData();
      final userData = cached ?? await UserPermissionService.loadUserData();
      ...
      final canAssign = _checkCanAssign(userData);

      // TabController yalnızca burada bir kez oluşturuluyor
      _tabController = TabController(
        length: canAssign ? 2 : 1,
        vsync: this,
      );
      ...
      debugPrint('ToDoListScreen yükleme hatası: $e');
      if (!mounted) return;
      // Hata durumunda yine de 1 sekmeyle aç
  ...
  /// Görev atayabilir mi? (Yönetici rolleri)
  bool _checkCanAssign(Map<String, dynamic>? data) {
    if (data == null) return true; // sistem admin
  ...
  Widget build(BuildContext context) {
    // Yükleme sırasında basit loading ekranı
  ...
  Widget _buildTaskList({required bool isMyTask}) {
    ...
    if (_activeTermId != null) {
      query = query.where('termId', isEqualTo: _activeTermId);
    }
      ...
      builder: (context, snapshot) {
        ...
        if (docs.isEmpty) {
          return _buildEmptyState(
            isMyTask
                ? 'Bu dönemde size atanmış görev bulunmuyor.'
                : 'Bu dönemde verdiğiniz görev bulunmuyor.',
        ...
        for (var doc in docs) {
          try {
            ...
            debugPrint('Görev parse hatası (${doc.id}): $e');
          }
        }

        final topPadding = _canAssign ? 120.0 : 90.0;
  ...
  Widget _buildModernTaskCard(ToDoTask task, {required bool isMyTask}) {
    ...
    final double progress =
        totalAssignees > 0 ? completionCount / totalAssignees : 0.0;
    ...
    final cardColor = isCompletedByMe ||
            (!isMyTask && completionCount == totalAssignees && totalAssignees > 0)
        ? Colors.grey.shade50
        : Colors.white;
  ...
  Future<void> _toggleTaskCompletion(ToDoTask task) async {
    ...
    if (isCompleted) {
      // Tamamlamayı geri al — hem completedBy hem completedAt'ten çıkar
      await docRef.update({
        'completedBy': FieldValue.arrayRemove([_currentUserId]),
        'completedAt.$_currentUserId': FieldValue.delete(),
      });
    } else {
      // Tamamla — hem completedBy'a ekle hem tamamlama zamanını kaydet
      await docRef.update({
        'completedBy': FieldValue.arrayUnion([_currentUserId]),
        'completedAt.$_currentUserId': FieldValue.serverTimestamp(),
      });
    }
  }
}
