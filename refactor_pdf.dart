...
void main() {
  ...
  pw.Widget _buildSummaryAveragesTable(Map<dynamic, dynamic> nets, Map<dynamic, dynamic> counts) {
    ...
    final row1 = ['ORTALAMA SORU SAYISI'];
    final row2 = ['ORTALAMA NET'];
    ...
    row1.add(totalQ.toString());
    row2.add(totalNet.toStringAsFixed(2));
...
""";
  ...
  final startKey = 'pw.Widget _buildPremiumFirstPage';
  final endKey = 'pw.Widget _dashboardCard';
  
  int startIdx = content.indexOf(startKey);
  int endIdx = content.indexOf(endKey);

  if (startIdx != -1 && endIdx != -1) {
    content = content.substring(0, startIdx) + newFirstPage + "\n\n  " + content.substring(endIdx);
  }

  // 2. Replace _buildTrialExamsSection
  String newTrialExamsSection = """
  pw.Widget _buildTrialExamsSection(Map<String, dynamic> data, bool newPage, pw.Font fontBold) {
    ...
    final subjects = ['T.NET', 'TRK', 'MAT', 'FEN', 'SOS', 'İNG', 'DİN'];
    ...
    if (topicStats.isEmpty) {
      ...
      List<dynamic> tableData = [[\'DERS / KONU ADI\', \'SS\', \'D\', \'Y\', \'B\', \'NET\', \'BAŞARI\']];
      
      topicStats.forEach((subj, topics) {
        tableData.add([pw.Text(subj.toUpperCase(), style: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: PdfColors.indigo900)), '', '', '', '', '', '']);
        ...
        (topics as Map).forEach((tName, stats) {
          ...
          tableData.add([
             pw.Padding(padding: const pw.EdgeInsets.only(left: 10), child: pw.Text(tName.toString(), style: const pw.TextStyle(fontSize: 8))),
             ss.toString(),
             stats['d'].toString(),
             stats['y'].toString(),
             stats['b'].toString(),
...
""";
  ...
  final trialStartKey = 'pw.Widget _buildTrialExamsSection';
  final trialEndKey = 'pw.Widget _buildInterviewsSection';

  int tStartIdx = content.indexOf(trialStartKey);
  int tEndIdx = content.indexOf(trialEndKey);

  if (tStartIdx != -1 && tEndIdx != -1) {
    content = content.substring(0, tStartIdx) + newTrialExamsSection + "\n\n  " + content.substring(tEndIdx);
  }
